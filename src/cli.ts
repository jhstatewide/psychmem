...
const agentType = 'opencode' as const;
const args = rawArgs;
...
async function handleInstall(installArgs: string[]) {
  if (installArgs.length > 0 && !installArgs.includes('--opencode')) {
    console.error('Unsupported install flags. Use `psychmem install` or `psychmem install --opencode`.');
    process.exit(1);
  }
...
/**
 * Normalize hook input to PsychMem internal format.
 */
function normalizeHookInput(raw: Record<string, unknown>): HookInput {
  if (raw.hook_event_name || raw.transcript_path || raw.session_id) {
    throw new Error('Claude Code hook payloads are no longer supported. Provide PsychMem HookInput JSON.');
  }

  if (!raw.hookType || !raw.sessionId || !raw.data) {
    throw new Error('Invalid hook input. Expected HookInput with hookType, sessionId, timestamp, and data.');
  }

  const input = raw as unknown as Partial<HookInput>;
  return {
    hookType: input.hookType as HookInput['hookType'],
    sessionId: input.sessionId as string,
    timestamp: input.timestamp ?? new Date().toISOString(),
    data: input.data as HookInput['data'],
  };
}
