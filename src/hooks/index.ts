...
export class PsychMemHooks {
  ...
  /**
   * UserPromptSubmit: Capture user input for potential memory extraction
   * Note: This is a lightweight hook - main processing happens at Stop
   */
 ...
 */
export async function createPsychMemHooks(config: Partial<PsychMemConfig> = {}): Promise<PsychMemHooks> {
  const hooks = new PsychMemHooks(config);
  await hooks.init();
  return hooks;
}
